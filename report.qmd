...
Across a general trend, we hypothesize movies that fail the Bechdel test will have higher success. We define greater success as higher ratings, greater gross income and more awards. Although there has been progress in creating larger roles for women in film, society still values certain gender stereotypes in films and some of these still do very well in the box office.
...
This graph shows the number of movies produced each year in our data set. As seen above, there is a greater number of movies produced in since the year 2000. This may lead to some years having a greater number of movies that pass the Bechdel test than others. While we do not directly use this to draw conclusions, it is useful in giving an overview of the data we are using.
...
Along with our explanatory data analysis, we decided to conduct hypothesis tests to conclude our data. We used hypothesis testing because, in our research question, we are trying to determine if the Bechdel test influences success. The Chi-Squared Test for Independence, Fisher's Test, and the Difference in Means Hypothesis Test all determine if there is influence from one variable to another. Specifically, the Chi-Squared Test and Fisher's Test determine if two variables are dependent on each other and The Difference in Means Hypothesis Test determines if the means are different based on if the movie passes or fails the Bechdel test. These tests will give us good insight into if there is a relationship between the Bechdel test and success.
...
Despite the lack of evidence in awards, we found that the overall success regarding ratings and income in movies increases when such movies fail the Bechdel Test. From our low p-values in both our hypothesis tests and chi-squared tests, we have significant evidence that movies that fail the Bechdel Test have higher ratings and income than those that do. On the other hand, as awards are chosen by a small committee with certain guidelines in place, it makes sense for them not to show a significant difference regarding the Bechdel Test as many biases and politics are at play. One the other hand, ratings and income are dictated by the overall viewers showing the greater societal values and trends in viewership.

For the future, there are many other variables we can explore regarding the Bechdel test in our data such as certain actors, director, country, language, and a description of plot. Perhaps exploring how culture affects the Bechdel test results through language and country would be more insightful than just looking at all movies. In addition, there could be some third variable problems inside our results as other variables could explain success in movies along with the Bechdel test. Looking at a linear regression across all our variables would help to explain this issue.
